...
class ARCoreSessionLifecycleHelper(
  ...
  override fun onPause(owner: LifecycleOwner) {
    Log.d("ARCoreHelper", "onPause called. Attempting to pause ARCore session.")
    try {
      session?.pause()
      Log.d("ARCoreHelper", "ARCore session paused successfully.")
    } catch (e: Exception) {
      // Even though the specific error is a CameraAccessException,
      // ARCore might wrap it. Catching a general Exception here for logging.
      Log.e("ARCoreHelper", "Exception during session.pause(): ${e.message}", e)
      // Rethrow if necessary, or handle as per your app's logic
      // The CameraAccessException will likely propagate up if not caught by ARCore internally.
    }
  }
