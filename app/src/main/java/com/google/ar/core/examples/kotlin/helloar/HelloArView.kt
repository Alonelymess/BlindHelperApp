...
class HelloArView(context: Context, attrs: AttributeSet?) : RelativeLayout(context, attrs), DefaultLifecycleObserver {
  ...
  init {
    Log.d("HelloArView", "Constructor init started.")
    // Ensure the view is used within the correct activity.
    if (context !is HelloArActivity) {
      throw IllegalStateException("HelloArView must be used within a HelloArActivity context.")
    }
    activity = context

    LayoutInflater.from(context).inflate(R.layout.hello_ar_view_layout, this, true)
    Log.d("HelloArView", "Internal layout inflated successfully.")

    surfaceView = findViewById(R.id.surfaceview)
    settingsButton = findViewById(R.id.settings_button)
    Log.d("HelloArView", "Child views found successfully.")

    // Initialize helpers that depend on the activity or views.
    tapHelper = TapHelper(activity).also { surfaceView.setOnTouchListener(it) }
    snackbarHelper.setParentView(this) // Use 'this' as the parent view for the snackbar.

    // Set up the listener for the settings button.
